(() => {
  ...
  function syncWinRates() {
    document.querySelectorAll('.rp-profile-record').forEach((record) => {
      ...
      let rateLabel = record.querySelector(':scope > .rp-profile-win-rate-label');
      if (!rateLabel) {
        rateLabel = document.createElement('span');
        rateLabel.className = 'rp-profile-win-rate-label';
        rateLabel.textContent = 'WIN RATE';
        record.appendChild(rateLabel);
      }
      ...
      meta.textContent = `${games} GAME${games === 1 ? '' : 'S'}`;
    });
  }
