(() => {
  ...
  function createPanel() {
    ...
    panel.innerHTML = `
      <div class="rp-updates-shell">
        ...
        <section class="rp-updates-head"><small>REAL PLAY</small><h1>WHAT'S HAPPENING.</h1><p>Official schedules, results, and announcements from Real Play.</p></section>
        ...
        <div class="rp-updates-admin" data-updates-admin hidden>
          ...
          <form class="rp-updates-admin-form" data-updates-admin-form hidden>
            <div class="rp-updates-admin-grid">
              <label>TYPE<select name="category" required><option value="announcement">ANNOUNCEMENT</option><option value="schedule">SCHEDULE</option><option value="result">RESULT</option></select></label>
              <label>DATE / TIME<input type="datetime-local" name="eventAt" /></label>
            </div>
            <label>TITLE<input name="title" maxlength="140" placeholder="WHAT PLAYERS NEED TO KNOW" required /></label>
            <label>DETAILS<textarea name="body" rows="4" maxlength="2400" placeholder="Add the official update details..."></textarea></label>
            <label>LOCATION<input name="locationName" maxlength="180" placeholder="Optional court or venue" /></label>
            <label class="rp-updates-pin"><input type="checkbox" name="pinned" /> PIN THIS UPDATE</label>
            <div class="rp-updates-admin-actions"><button type="submit" data-updates-publish>PUBLISH</button><button type="button" data-updates-admin-cancel>CANCEL</button></div>
          </form>
        </div>
    ...
    panel.querySelectorAll('[data-update-filter]').forEach((button) => button.addEventListener('click', () => {
      filter = button.dataset.updateFilter || 'all';
      renderFilters();
      renderFeed();
    }));
    panel.querySelector('[data-updates-admin-open]')?.addEventListener('click', () => {
      panel.querySelector('[data-updates-admin-form]').hidden = false;
  ...
  function renderFilters() {
    panel?.querySelectorAll('[data-update-filter]').forEach((button) => button.classList.toggle('active', button.dataset.updateFilter === filter));
  }
  ...
  function renderFeed() {
    ...
    root.innerHTML = visible.map((update) => `
      <article class="rp-update-card rp-update-${esc(update.category)}${update.pinned ? ' pinned' : ''}" data-update-id="${esc(update.id)}">
        <header class="rp-update-card-head"><div class="rp-update-kind"><span>${categoryIcon(update.category)}</span><div><strong>${categoryLabel(update.category)}</strong><small>${esc(timeAgo(update.published_at || update.publishedAt))}</small></div></div>${update.pinned ? '<b class="rp-update-pin">PINNED</b>' : ''}</header>
  ...
  async function handleFeedClick(event) {
    ...
    if (!id || !confirm('Delete this manual Real Play update?')) return;
    button.disabled = true;
    try {
