(() => {
  ...
  function timeLabel(value) {
    ...
    return new Intl.DateTimeFormat('en-PH', { month: 'short', day: 'numeric', timeZone: 'Asia/Manila' })
      ...
      <div class="rp-world-shell">
        ...
        <main class="rp-world-body">
          ...
          <section class="rp-world-view" data-world-view="chats" hidden>
            <div class="rp-chat-heading"><div><small>REAL PLAY CHAT</small><h1>Talk to your people.</h1></div><span>TEAM + WORLD</span></div>
            <div class="rp-chat-channels" data-chat-channels></div>
            <div class="rp-chat-thread-wrap">
              <header class="rp-chat-thread-head"><div><strong data-chat-title>WORLD CHAT</strong><span data-chat-subtitle>Everyone in Real Play</span></div><b>●</b></header>
              <div class="rp-chat-thread" data-chat-thread></div>
              <form class="rp-chat-form" data-chat-form><textarea rows="1" maxlength="1000" placeholder="Message World Chat..." data-chat-input required></textarea><button type="submit" aria-label="Send message" data-chat-send>↑</button></form>
            </div>
      ...
      return `
        <article class="rp-world-post" data-post-id="${esc(post.id)}">
          ...
          <div class="rp-world-post-actions">
            <button type="button" class="${post.likedByMe ? 'active' : ''}" data-post-like><span>♥</span> ${Number(post.likeCount || 0)}</button>
            <button type="button" data-post-comment-toggle><span>●</span> ${comments.length ? `${comments.length} COMMENT${comments.length === 1 ? '' : 'S'}` : 'COMMENT'}</button>
          </div>
  ...
  async function refreshChat({ quiet = true, scroll = false } = {}) {
    ...
    try {
      const data = await community('chat', { channel: currentChannel || 'world' });
  ...
  async function handleFeedSubmit(event) {
    ...
    const body = String(input?.value || '').trim();
    if (!body || !post?.dataset.postId || !button) return;
    button.disabled = true;
    try {
      const data = await community('create_comment', { postId: post.dataset.postId, body });
      feed = Array.isArray(data.feed) ? data.feed : feed;
      renderFeed();
    } catch (error) {
  ...
  function setWorldNavActive(active) {
    document.querySelectorAll('[data-rp-bottom-nav] .rp-nav-item').forEach((button) => button.classList.toggle('active', active && button.dataset.rpNav === 'world'));
  ...
  if (!repurposeNavigation()) {
    const observer = new MutationObserver(() => { if (repurposeNavigation()) observer.disconnect(); });
